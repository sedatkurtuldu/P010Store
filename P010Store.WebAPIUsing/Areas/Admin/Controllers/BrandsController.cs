...
namespace P010Store.WebAPIUsing.Areas.Admin.Controllers
{
	...
	public class BrandsController : Controller
	{
		...
		private readonly string _apiAdres = "https://localhost:7212/api/Brands";
		...
		public async Task<IActionResult> CreateAsync(Brand brand, IFormFile? Logo)
		{
			if (ModelState.IsValid)
			{
				try
				{
					...
					if(response.IsSuccessStatusCode) return RedirectToAction(nameof(Index)); // eğer response değişkeni API'den IsSuccessStatusCode yani başarılı bir durum kodu dönmüşse sayfayı index'e yönlendir.
				}
				catch
				{
